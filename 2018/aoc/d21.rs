...
fn looper(ip: usize, ins: &Vec<Func>) -> (usize, usize) {
...
fn main() -> std::io::Result<()> {
    let (ip, instructions) = fs::read_to_string("in/d21.txt")?
        ...
        .map(|(rip, rins)| {
            ...
            let ins = rins.lines().map(|r| convert(r)).collect::<Vec<Func>>();
            (ip, ins)
        })
        .unwrap();
