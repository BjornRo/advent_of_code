...
const Block = struct {
    ...
    fn add(self: *Self, o: u8) void {
        ...
        self.count += 1;
        self.available = self.size - self.count;
    }
    fn initNull(size: u8, available: u8) Self {
        return .{ .size = size, .count = 0, .elems = [_]u16{0}, .available = available };
    }

    fn initFile(size: u8, id: u16) Self {
        return .{ .size = size, .count = 0, .elems = [size]u16{id}, .available = available };
    }
};
