...
dot_prod = lambda u, v: Decimal(sum(ui * vi for ui, vi in zip(u, v)))
vec_len = lambda u: Decimal(sum(map(lambda x: x**2, u))).sqrt()
...
def part1(isbollar: tuple[Isboll, ...], test_min_xy: int, test_max_xy: int) -> int:
    ...
    for i, a in enumerate(isbollar):
        for b in isbollar[i + 1 :]:
            if not is_parallel(a, b) and intersects(a, b):
