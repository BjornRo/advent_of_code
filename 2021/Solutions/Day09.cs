...
public class Day09
{
    ...
    static IEnumerable<(int, int)> Cross3(byte[][] matrix, int row, int col)
    {
    ...
    static Dictionary<Point, int> Part1(byte[][] matrix)
    {
        ...
        for (int i = 0; i < matrix.Length; i++)
            for (int j = 0; j < matrix[0].Length; j++)
                if (Cross3(matrix, i, j).All(x => matrix[x.Item1][x.Item2] > matrix[i][j]))
            ...
            .Select(k =>
                {
                    ...
                    while (queue.TryDequeue(out var p))
                        foreach (var np in Cross3(matrix, p.Row, p.Col).Select(x => new Point(x.Item1, x.Item2)))
